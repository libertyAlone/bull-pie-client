...
自选里**也可以放港股和美股**（代码以 `.HK` / `.US` 结尾，例如 `3308.HK`、`AAPL.US`）：A 股 / 场内基金
走同花顺，港股 / 美股走长桥，全部显示在同一个列表里，价格后面会标出货币（`HKD` / `USD`，A 股不标）。
这两类行情需要先在「港美股」页点一次「用长桥账号登录」——没登录时 A 股那部分照常显示，港股 / 美股行会
显示「—」并在表格上方给一句提示。

> 刷新节奏跟着 A 股：**港股 15:05–16:00 与美股夜盘时段不会自动刷新**，想看最新价点一下「立即刷新」即可。

加进来的港股会**自动同步到「港美股」页的自选**（两边是同一只票，省得再加一次）；反过来，
在「港美股」页加的港股不会自动进 A 股自选——那是两块各自独立的关注列表。要从「港美股」自选里
去掉它，在那一页点移除即可（在 A 股自选里删不会连带删除）。

要一次加很多只（比如把券商 App 里的持仓导出来）时，用自选页里的 **「批量导入自选」**（默认收起，点「展开」）：

1. **上传表格**：支持 CSV / TSV / txt（Excel 请先「另存为 CSV」，UTF-8 或 GBK 都能读），或者直接把一列
   代码 / 名称**粘**进文本框再点「解析粘贴内容」；表头、序号、涨跌幅这些列会自动跳过，代码与名称都能认
   （`600519`、`600519.SH`、`SH600519`、`贵州茅台` 都行）。
2. **看一眼命中情况**：解析与匹配都在你本机完成（只查本地代码表，不联网、不上传）。命中的会列出代码与名称；
   同名多只（如 A/H 同名）会标成「有歧义」，需要你手动搜索确认；本地代码表里没有的会标「未匹配」——
   多半是代码表还没同步（去 **设置 → 数据 → 同步代码表**）或名称写错了。
3. **选分组后导入**：可以指定导入到哪个分组（默认分组之外的会一并挪过去），点「导入 N 只」完成。

如果你手上只有**截图**（券商 App 的持仓页、别人发的清单），用同一张卡片的第三块「从截图识别」：

1. 点「选择截图…」或直接在框里 **Ctrl+V** 粘贴剪贴板里的图；
2. 点「识别并解析」。这一步需要先在「AI 助手」页配好**能看图的模型**（本地 Ollama 或订阅制接口都行），
   图片会发到那个模型服务（订阅制按次计费；本地 Ollama 不出本机）；
3. 模型**只负责把图里的代码与名称原样抄出来**，代码一律由本地代码表核对——抄错、看不清、同名多只的
   都会落在「未匹配 / 有歧义」里，不会凭空多出一只，也不会替你决定买哪只。
...
> 不想把过去的每一笔成交都补上？在「记一笔流水」里把**流水类型选成「买入」**，勾上下面的
> **「按当前持仓补录」**：数量填**现在实际持有多少股**、价格填**持仓成本价**（不是现价）即可。
> 保存后会自动生成两条流水——一笔等额入金加一笔买入——所以现金不会被记成负数；投入本金算作你的
> 持仓成本，**收益率从这一天开始算，此前的盈亏不计入本账本**。适合「从今天起重新跟踪」的场景。
...
- **最新价按当前时段**：盘前、盘后、夜盘哪一段正在走，个股详情的「最新价 / 涨跌额 / 涨跌幅」就用那一段的实时价（页头数据时间后面会标「（夜盘实时）」），与富途等手机 App 一致；不在延长时段时用常规时段价。**自选列表里不一样**：主价固定显示常规时段收盘价，右边小字才是当前时点的价与它属于哪一段（盘前 / 盘中 / 盘后 / 夜盘；都不在走时就是最近收掉的那一段，常见是上一个交易日的盘后），涨跌额 / 涨跌幅按常规时段算——主价不会因为夜盘开了就整列换口径。价格旁标货币（`HKD` / `USD`），免得同一列里看混。
- **分时时段切换**：美股把分时入口做成了详情卡头部的下拉（显示为「分时 · 夜盘」这种形式，点开是 盘前 / 盘中 / 盘后 / 夜盘 / 全天）——选哪一段会直接切到分时并画那一段，不用先点「分时」tab（所以美股那排 tab 只有 K 线 / 基本面；港股没有延长时段，仍用「分时」tab），横轴会换成那一段当地时间；鼠标移到图上会显示该时点的价、**涨跌幅**、均价与该分钟成交量（涨跌幅相对该时段的基准：盘前 / 盘中是昨收，盘后 / 夜盘是当日常规收盘，图例上的叫法也会跟着换）。下拉里每一项右边同时标着该时段的当地时间范围与**该段自己的涨跌幅**（相对**上一场常规收盘**：盘前、盘中相对昨收，盘后、夜盘相对当日常规收盘——所以夜盘那格显示的是「夜盘走了多少」，不是当日累计）。**当前不在该时段时**（例如夜盘时段点「盘前」）自动显示**上一个交易日同一时段**的分时，并在下拉旁写明是哪一天（夜盘时段上游不返回盘前 / 盘后的数据，这一份是单独回查历史分钟线拿到的）。
